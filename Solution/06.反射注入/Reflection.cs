...
namespace JFramework
{
    internal static class Reflection
    {
        ...
        static Reflection()
        {
            ...
            toolbarRoot = toolbarType.GetField("m_Root", Utility.Find.Instance);
            activeHierarchy = hierarchyType.GetField("m_SceneHierarchy", Utility.Find.Instance);
            selectHierarchy = hierarchyType.GetField("s_LastInteractedHierarchy", Utility.Find.Static);
            inspectorTracker = inspectorType.GetProperty("tracker", Utility.Find.Instance);

            var editorType = Utility.Find.Assembly("UnityEditor.CoreModule").GetType("UnityEditor.UIElements.EditorElement");
            editorElements = editorType.GetProperty("m_Editors", Utility.Find.Instance);
            editorType = GetEditorType("SceneHierarchy");
            hierarchyTree = editorType.GetField("m_TreeView", Utility.Find.Instance);
            editorType = GetEditorType("IMGUI.Controls.TreeViewGUI");
            iconWidth = editorType.GetField("k_IconWidth", Utility.Find.Instance);
            iconLabel = editorType.GetField("k_SpaceBetweenIconAndText", Utility.Find.Instance);
            editorType = GetEditorType("ProjectBrowser");
            selectProject = editorType.GetField("s_LastInteractedProjectBrowser", Utility.Find.Static);
            itemTreeViewA = editorType.GetField("m_AssetTree", Utility.Find.Instance);
            itemTreeViewB = editorType.GetField("m_FolderTree", Utility.Find.Instance);
            editorType = GetEditorType("IMGUI.Controls.TreeViewController");
            itemTreeViewC = editorType.GetProperty("gui", Utility.Find.Instance);
            itemTreeViewD = editorType.GetProperty("data", Utility.Find.Instance);
            editorType = GetEditorType("ProjectBrowserColumnOneTreeViewDataSource");
            itemTreeViewE = editorType.GetMethod("GetRows", Utility.Find.Instance);
            editorType = GetEditorType("AssetsTreeViewDataSource");
            itemTreeViewF = editorType.GetMethod("GetRows", Utility.Find.Instance);
            editorType = Utility.Find.Assembly("JFramework.Unity")?.GetType("JFramework.EditorSetting");
            showEditorWindow = editorType?.GetMethod("ShowWindow", Utility.Find.Static);
            var types = new[] { typeof(Rect), typeof(Object), typeof(int) };
            contextMenuMethod = GetMethod(typeof(EditorUtility), Utility.Find.Static, "DisplayObjectContextMenu", types);
